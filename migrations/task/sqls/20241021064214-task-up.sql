...
select
	"USER".id as user_id,
	(SUM("CREDIT_PURCHASE".purchased_credits) - COUNT("COURSE_BOOKING")) as remaining_credit
from "USER"
inner join "CREDIT_PURCHASE" on "CREDIT_PURCHASE".user_id = "USER".id
inner join "COURSE_BOOKING" on "COURSE_BOOKING".user_id = "USER".id
where 
	"USER".id = (select id from "USER" where name = '王小明')
	and
	"COURSE_BOOKING".status not in ('課程已取消')
group by "USER".id;
...
SELECT
	"SKILL".name AS 專長名稱,
	COUNT("COACH_LINK_SKILL") as coach_total
FROM "SKILL"
INNER JOIN "COACH_LINK_SKILL" ON "SKILL".id = "COACH_LINK_SKILL".skill_id
group by "SKILL".name
order by COUNT("COACH_LINK_SKILL") desc
limit 1;
